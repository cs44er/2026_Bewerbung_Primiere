document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ============================================
     PRAXIS-SECTION: STAGGERED CARD-REVEAL
     ============================================ */
  const praxisGrid = document.getElementById('praxisGrid');
  const praxisCards = gsap.utils.toArray('.card-symptom');

  if (praxisCards.length) {
    if (reduceMotion) {
      gsap.set(praxisCards, { opacity: 1, y: 0, scale: 1 });
    } else {
      gsap.set(praxisCards, { opacity: 0, y: 40, scale: 0.96 });
      ScrollTrigger.create({
        trigger: praxisGrid || praxisCards[0],
        start: 'top 85%',
        once: true,
        onEnter: () => {
          gsap.to(praxisCards, {
  ...
  /* ============================================
     WORKFLOW-DIAGRAMM (Gedankenspiel für Bahmann)
     ============================================ */
  const dots = {
    off: '#23262E',
    on: '#4CFFA0'
  ...
  /* ============================================
     ROUTE-DIAGRAMM (Section: Persönlich)
     ============================================ */
  const routeDot1 = document.getElementById('routeDot1');
  const routeConn = document.getElementById('routeConn');
  const routeDot2 = document.getElementById('routeDot2');

  if (routeDot1 && routeConn && routeDot2) {
    if (reduceMotion) {
      gsap.set([routeDot1, routeDot2], { backgroundColor: dots.on });
      gsap.set(routeConn, { scaleX: 1 });
    } else {
      gsap.set([routeDot1, routeDot2], { backgroundColor: dots.off });
      gsap.set(routeConn, { scaleX: 0 });
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          tl.to(routeDot1, { backgroundColor: dots.on, boxShadow: '0 0 16px 4px rgba(76,255,160,0.45)', duration: 0.3 })
            .to(routeConn, { scaleX: 1, duration: 0.6, ease: 'none' })
            .to(routeDot2, { backgroundColor: dots.on, boxShadow: '0 0 16px 4px rgba(76,255,160,0.45)', duration: 0.3 });
        }
      });
    }
  }
